...
import { Plus, Pencil, Trash2, X, AlertTriangle, FileSpreadsheet } from 'lucide-react';
...
export default function Produtos() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([produtoService.listar(), fornecedorService.listar()])
      .then(([p, f]) => { setItems(p.data); setFornecedores(f.data); })
  ...
  const openEdit   = (item) => {
    setEditing(item);
    setForm({ ...item, preco: item.preco, fornecedor: item.fornecedor ? { id: item.fornecedor.id } : null });
    setErrors({});
    setModal(true);
  };
  ...
  return (
    <div>
      {/* ✅ Topbar com ambos os botões corretamente no actions */}
      <Topbar
        title="Produtos"
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            {semEstoque > 0 && (
              <button className={`btn btn-sm ${filterLow ? 'btn-danger' : 'btn-ghost'}`}
                onClick={() => setFilterLow(f => !f)} style={{ gap: 5 }}>
                <AlertTriangle size={13} /> Sem estoque ({semEstoque})
              </button>
            )}
          </div>
        ...
        {loading ? (
          ...
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Produto</th><th>Cód.</th><th>Preço</th>
                  <th>Estoque</th><th>Fornecedor</th><th>Status</th><th>Ações</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(item => (
                  <tr key={item.id}>
                    ...
                    <td>
                      <span style={{ fontWeight: 700, color: item.estoque === 0 ? 'var(--danger)' : item.estoque <= 5 ? 'var(--warning)' : 'var(--text)' }}>
            ...
            <form onSubmit={handleSubmit} noValidate>
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input className={`form-input${errors.nome ? ' error' : ''}`} value={form.nome}
                    onChange={e => setField('nome', e.target.value)} placeholder="Nome do produto" />
                  {errors.nome && <div className="field-error">{errors.nome}</div>}
                </div>
                <div className="form-group">
                  <label className="form-label">Código Interno</label>
                  <input className="form-input" value={form.codigoInterno}
                    onChange={e => setField('codigoInterno', e.target.value)} placeholder="SKU-001" />
                </div>
              </div>
              ...
              <div className="form-group">
                ...
                <textarea className="form-textarea" value={form.descricao}
                  onChange={e => setField('descricao', e.target.value)} placeholder="Descrição detalhada…" />
              </div>

              <div className="form-row">
                <div className="form-group">
                  <label className="form-label">Preço (R$) *</label>
                  <input className={`form-input${errors.preco ? ' error' : ''}`} type="number"
                    min="0" step="0.01" inputMode="decimal" value={form.preco}
                    onChange={e => setField('preco', e.target.value)} placeholder="0.00" />
                  {errors.preco && <div className="field-error">{errors.preco}</div>}
                </div>
                <div className="form-group">
                  <label className="form-label">Estoque</label>
                  <input className={`form-input${errors.estoque ? ' error' : ''}`} type="number"
                    min="0" value={form.estoque} onChange={e => setField('estoque', e.target.value)} />
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input className="form-input" value={form.categoria}
                    onChange={e => setField('categoria', e.target.value)} placeholder="Ex: Eletrônicos" />
                </div>
                <div className="form-group">
              ...
              <div className="form-group">
                ...
                <select className="form-select" value={form.fornecedor?.id || ''}
