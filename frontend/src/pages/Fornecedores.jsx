...
const EMPTY = { nome: '', cnpj: '', contato: '', email: '', telefone: '', endereco: '', cidade: '', estado: '', categoria: '', ativo: true };
...
export default function Fornecedores() {
  ...
  const load = () => {
    setLoading(true);
    fornecedorService.getAll().then(r => setItems(r.data)).catch(() => toast.error('Erro ao carregar')).finally(() => setLoading(false));
  };
  useEffect(() => { load(); }, []);
  ...
  const handleSubmit = async (ev) => {
    ...
    setSaving(true);
    try {
      if (editing) { await fornecedorService.update(editing.id, form); toast.success('Fornecedor atualizado!'); }
      else         { await fornecedorService.create(form);              toast.success('Fornecedor cadastrado!'); }
      closeModal(); load();
  ...
  const handleDelete = async (id) => {
    ...
    try { await fornecedorService.delete(id); toast.success('Removido!'); load(); }
    catch (err) { toast.error(err.message); }
  };
  ...
  return (
    <div>
      <Topbar
        ...
        actions={<button className="btn btn-primary" onClick={openCreate}><Plus size={15} /> Novo Fornecedor</button>}
      />
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <div className="empty"><div className="empty-icon">🚚</div><h3>Nenhum fornecedor</h3><p>Clique em "Novo Fornecedor" para adicionar</p></div>
        ) : (
          <div className="table-wrap">
            <table>
              <thead><tr>
                <th>Nome</th><th>CNPJ</th><th>Contato</th><th>Telefone</th><th>Categoria</th><th>Cidade / UF</th><th>Status</th><th>Ações</th>
              </tr></thead>
              <tbody>
                {filtered.map(item => (
                  <tr key={item.id}>
                    ...
                    <td>{item.categoria ? <span className="badge badge-orange">{item.categoria}</span> : <span style={{ color: 'var(--text3)' }}>—</span>}</td>
            ...
            <form onSubmit={handleSubmit} noValidate>
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input className={`form-input${errors.nome ? ' error' : ''}`} value={form.nome} onChange={e => setField('nome', e.target.value)} placeholder="Nome do fornecedor" />
                  {errors.nome && <div className="field-error">{errors.nome}</div>}
                </div>
                <div className="form-group">
                  <label className="form-label">CNPJ</label>
                  <input className="form-input" value={form.cnpj} onChange={e => setField('cnpj', formatCNPJ(e.target.value))} placeholder="00.000.000/0001-00" />
                </div>
              </div>
              <div className="form-row">
                <div className="form-group">
                  <label className="form-label">Contato</label>
                  <input className="form-input" value={form.contato} onChange={e => setField('contato', e.target.value)} placeholder="Nome do responsável" />
                </div>
                <div className="form-group">
                  <label className="form-label">Email</label>
                  <input className={`form-input${errors.email ? ' error' : ''}`} type="email" value={form.email} onChange={e => setField('email', e.target.value)} placeholder="email@fornecedor.com" />
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" value={form.categoria} onChange={e => setField('categoria', e.target.value)} placeholder="Ex: Eletrônicos, Alimentos…" />
                </div>
              </div>
              <div className="form-group">
                <label className="form-label">Endereço</label>
                <input className="form-input" value={form.endereco} onChange={e => setField('endereco', e.target.value)} placeholder="Endereço completo" />
              </div>
              <div className="form-row">
                <div className="form-group">
